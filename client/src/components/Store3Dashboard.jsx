...
const Store1Dashboard = () => {
  ...
  const [wh3Stock, setWh3Stock] = useState(0);
  const [wh3Revenue, setWh3Revenue] = useState(0);
  ...
  useEffect(() => {
    const fetchRevenue = async () => {
      try {
        const response = await axios.get("http://localhost:3002/revenue/warehouse/WH-3");
  ...
  useEffect(() => {
    const fetchStock = async () => {
      try {
        ...
        const wh3 = response.data?.breakdown?.["WH-3"];
          ...
          <div className="mt-auto p-5 bg-white/5 border border-white/10 rounded-3xl backdrop-blur-md">
             <div className="flex items-center gap-3">
                ...
                <div>
                   ...
                   <p className="text-[10px] text-slate-400 uppercase font-black tracking-tighter">Auth Store 3</p>
                </div>
             </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative z-10">
        ...
        <main className="p-8 lg:p-12 space-y-10 overflow-y-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <StatCard label="Total Revenue" value={"$" + wh3Revenue.toLocaleString()} icon={<DollarSign/>} color="blue" />
            <StatCard label="Total Stocks" value={wh3Stock} icon={<Package/>} color="teal" />
            <StatCard
              label="Anomaly Count"
